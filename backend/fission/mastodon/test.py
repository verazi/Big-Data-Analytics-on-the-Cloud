...
def get_last_run_data() -> Tuple[datetime, Dict[str, str]]:
    ...
    try:
        ...
        if not es.indices.exists(index="mastodon-control"):
        ...
        if es.exists(index="mastodon-control", id="last_run"):
            result = es.get(index="mastodon-control", id="last_run")
        ...
        for tag in HASHTAGS:
            ...
            if es.exists(index="mastodon-control", id=tag_id):
                result = es.get(index="mastodon-control", id=tag_id)
...
def main():
    post_index = "mastodon-posts"
    global comment_index
    comment_index = "mastodon-comments"
